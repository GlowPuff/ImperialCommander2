...
namespace Saga
{
	public class SagaEventPopup : MonoBehaviour
	{
		...
		public void OnClose()
		{
			...
			fader.DOFade( 0, .5f ).OnComplete( () =>
			{
				...
				//handle any deployment
				if ( allyToAdd != null )
				{
					//only 1 event spawns an ally, and it doesn't spawn an enemy
					FindObjectOfType<DeploymentGroupManager>().DeployHeroAlly( allyToAdd );
					FindObjectOfType<SagaEventManager>().ShowTextBox( $"Deploy the following Ally:\n\n<color=white>{allyToAdd.name}</color> <color=orange>[{allyToAdd.id}]</color>" );
				}
				...
				if ( enemyToAdd != null )
				{
					...
					{
						FindObjectOfType<DeploymentManager>().DeployGroup( enemyToAdd );
						var ovrd = DataStore.sagaSessionData.gameVars.GetDeploymentOverride( enemyToAdd.id );
						//this also invokes callback when it's done
						FindObjectOfType<DeploymentManager>().HandleMapDeployment( enemyToAdd, callback, ovrd );
					}
				}
				else
					callback?.Invoke();
		...
		void ParseCard( string item )
		{
			...
			if ( cardEvent.eventRule == "R8" && item.Contains( "{V}" ) )
			{
				...
				if ( enemyToAdd != null )
				{
					...
					DataStore.sagaSessionData.ModifyThreat( -Math.Min( 7, enemyToAdd.cost ) );
				}
				else
			...
			else if ( cardEvent.eventRule == "R18" && item.Contains( "{CR}" ) )
			{
				...
				if ( enemyToAdd != null )
				{
					...
					DataStore.sagaSessionData.ModifyThreat( -Math.Min( 5, enemyToAdd.cost ) );
				}
				else
			...
			else if ( cardEvent.eventRule == "R23" )
			{
				...
				if ( allyToAdd != null )
				{
					...
					DataStore.sagaSessionData.ModifyThreat( allyToAdd.cost / 2 );
				}
				else
		...
		DeploymentCard HandleR8()
		{
			...
			*/
			...
			var v = DataStore.deploymentHand
				...
				.Where( x => x.cost <= DataStore.sagaSessionData.setupOptions.threatLevel + 7 ).ToList();
			if ( v.Count > 0 )
			{
			...
			if ( v.Count > 0 )
			{
				...
				DataStore.sagaSessionData.EarnedVillains.Add( v[rnd[0]] );
				return v[rnd[0]];
			}
			//bust
			return null;
